  ...
  function redraw() {
    ...
    var barEnter = bar
      ...
      .attr("transform", function (d) {
        return "translate(0," + (yScale(yValue(d)) + 1000) + ")";
      });
    ...
    barExit
      .attr("transform", function (d) {
        const thisBar = d3.select(this); // ✅ 올바르게 현재 요소를 선택
        const transform = thisBar.attr("transform"); // ✅ 이제 제대로 동작함
        const match = transform?.match(/translate\(0,\s*([-\d.]+)\)/);
        const y = match ? parseFloat(match[1]) : 0;

        console.log("ylocation:", y);
